...
class Environment(object):
    ...
    Then Environment return next state, reward, and so on.
    '''
    def __init__(self, discriminator,CNN, data_generator, g_beta, n_sample=16):
        '''
    ...
    def Q(self, action, n_sample=16):
        ...
        if self.t >= self.T+1:
            ...
            dis_result = self.discriminator.predict(Y)
            Y = sequence.pad_sequences(Y,maxlen=128,padding='post')
            
           
            cnn_result = self.CNN.predict(Y)
            
            return (dis_result+cnn_result[:,0:1]) / 2
        ...
        for _ in range(n_sample):
            ...
            cnn_result = self.CNN.predict(Y)
            reward += (dis_result +cnn_result[:,0:1])/ (n_sample*2)
