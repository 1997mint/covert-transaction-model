...
from keras.layers import Dense, Embedding, LSTM, Conv1D, GlobalMaxPooling1D,MaxPooling1D,concatenate,Flatten
...
def Discriminator_Cnn(V ,E , dropout=0.4):


    input = Input(shape=(128,), dtype='int32')
    # 词嵌入（使用预训练的词向量）
    embedder = Embedding(40, 300, trainable=False)
    ...
    flat = Flatten()(cnn)
    drop = Dropout(dropout,name='Dropout')(flat)

    output = Dense(2, activation='softmax')(drop)  # 修改输出空间维度
